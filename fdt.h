...
void fft1dIterRadix2(float* real, float* imag, unsigned L, bool inverse = false);
...
void dft1d(float* real, float* imag, unsigned L, bool inverse = false);

// 利用可分离性原地计算离散傅里叶变换和逆变换（unscaled）
void helpFFT(float* real, float* imag, int M, int N, bool inverse = false);
